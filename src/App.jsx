...
function signedAmount(tx) {
  if (!tx) return 0
  if (['withdraw', 'transfer_out'].includes(tx.type)) return -Math.abs(Number(tx.amount || 0))
...
function Dashboard({ session }) {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    if (p.role === 'admin') {
      const { data: userList } = await supabase.from('profiles').select('id, email, full_name, discord_id, discord_username').order('discord_username', { ascending: true })
      setUsers(userList || [])
    }
  ...
  function scheduleRefresh() {
    ...
    refreshTimer.current = setTimeout(() => { refreshTimer.current = null; loadData(true) }, 250)
  }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe()
    return () => { if (refreshTimer.current) clearTimeout(refreshTimer.current); supabase.removeChannel(channel) }
  ...
  async function doTransfer(e) {
    ...
    const ok = await runRpc('transfer_by_discord', { p_to_discord_id: transferToDiscord.trim(), p_amount: Number(transferAmount) })
    if (ok) { setTransferToDiscord(''); setTransferAmount('') }
  }

  async function doAdminAdjust(e) {
    e.preventDefault()
    if (!adminUserId || !adminAmount) return
    const ok = await runRpc('admin_adjust_user_balance', { p_user_id: adminUserId, p_amount: Number(adminAmount) })
  ...
  async function doSalary(e) {
    ...
    const ok = await runRpc('admin_add_salary', { p_user_id: salaryUserId, p_amount: Number(salaryAmount), p_source: salarySource, p_note: salaryNote })
    if (ok) { setSalaryAmount(''); setSalaryNote('') }
  }
  ...
  function parseTierDiscounts(raw) {
    ...
    }).filter(t => t.min_qty > 0 && t.discount_percent >= 0 && t.discount_percent <= 100).sort((a, b) => a.min_qty - b.min_qty)
  }

  function resolveDiscount(base, tiers, quantity) {
    let discount = Number(base || 0)
    ;(tiers || []).forEach(tier => { if (quantity >= Number(tier.min_qty || 0)) discount = Math.max(discount, Number(tier.discount_percent || 0)) })
  ...
  async function doCreateListing(e) {
    ...
    const { error } = await supabase.from('market_listings').insert({
      seller_id: session.user.id, title: listingTitle, description: listingDescription,
      unit_price: Number(listingPrice), stock: listingUnlimited ? null : Number(listingStock),
      is_unlimited: listingUnlimited, base_discount_percent: Number(listingBaseDiscount || 0),
      tier_discounts: tiers, is_active: true,
    })
    if (error) { setMessage(error.message); return }
    setListingTitle(''); setListingDescription(''); setListingPrice('')
    setListingStock('1'); setListingUnlimited(false); setListingBaseDiscount('0'); setListingTiers('')
    await loadData(true)
  }
  ...
  async function doBuy(listingId) {
    ...
    const ok = await runRpc('buy_market_listing', { p_listing_id: listingId, p_quantity: quantity })
    if (ok) setBuyQtyByListing(prev => ({ ...prev, [listingId]: 1 }))
  }

  async function signOut() { await supabase.auth.signOut(); window.location.reload() }

  const displayName = profile?.discord_username || profile?.full_name || profile?.email || session.user.email
    ...
    <div className="container">
      <header className="topbar">
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          {avatarUrl && <img src={avatarUrl} alt="avatar" style={{ width: 36, height: 36, borderRadius: '50%' }} />}
          <div>
            <h1>Banco de Umbrathel</h1>
            <p>{displayName} · Rol: <strong>{profile?.role || 'user'}</strong></p>
          </div>
        </div>
        ...
        <>
          <section className="grid">
            <form className="card" onSubmit={doTransfer}>
              ...
              <small style={{ color: '#888', marginTop: '-0.5rem' }}>Puedes ver tu ID en Discord con clic derecho → Copiar ID de usuario</small>
              ...
              <section className="card admin">
                ...
                <form onSubmit={doAdminAdjust}>
                  <select value={adminUserId} onChange={e => setAdminUserId(e.target.value)} required>
                    ...
                    {users.map(u => (
                      <option key={u.id} value={u.id}>{u.discord_username || u.email || u.id}</option>
                    ))}
                  </select>
          ...
          <section className="card">
            <h3>Movimientos</h3>
            {transactions.length === 0 ? <p>No hay movimientos aún.</p> : (
              <table>
      ...
      {tab === 'salary' && (
        <>
            ...
            <section className="card admin">
              ...
              <form onSubmit={doSalary}>
                <select value={salaryUserId} onChange={e => setSalaryUserId(e.target.value)} required>
                  <option value="">Selecciona usuario</option>
                  {users.map(u => <option key={u.id} value={u.id}>{u.discord_username || u.email || u.id}</option>)}
                </select>
          ...
          <section className="card">
            ...
            {salaryRecords.length === 0 ? <p>No hay pagos de salario aún.</p> : (
              <table>
                <thead><tr><th>Fecha</th><th>Origen</th><th>Nota</th><th>Monto</th></tr></thead>
                <tbody>
      ...
      {tab === 'market' && (
        <>
          <section className="card">
            ...
            <form onSubmit={doCreateListing}>
              ...
              <label className="inline-check">
                <input type="checkbox" checked={listingUnlimited} onChange={e => setListingUnlimited(e.target.checked)} /> Stock ilimitado
              </label>
              {!listingUnlimited && <input type="number" min="1" step="1" placeholder="Stock" value={listingStock} onChange={e => setListingStock(e.target.value)} required />}
              ...
              <input placeholder="Descuentos por lote (5:10,10:20)" value={listingTiers} onChange={e => setListingTiers(e.target.value)} />
          ...
          <section className="card">
            <h3>Mercado</h3>
            {visibleListings.length === 0 ? <p>No hay artículos publicados.</p> : (
                ...
                {visibleListings.map(item => {
                  ...
                  return (
                    <article className="market-item" key={item.id}>
                      ...
                      {tiers.length > 0 && <p className="tiny">Lotes: {tiers.map(t => `${t.min_qty}+ ⇒ ${t.discount_percent}%`).join(' | ')}</p>}
                      {!isOwn && item.is_active && (
                        <div className="buy-row">
                          <input type="number" min="1" step="1" value={buyQtyByListing[item.id] || 1} onChange={e => setBuyQtyByListing(prev => ({ ...prev, [item.id]: e.target.value }))} />
...
export default function App() {
  if (supabaseConfigError) {
    return <div className="auth-card"><h1>Banco de Umbrathel</h1><p className="message">{supabaseConfigError}</p></div>
  }
